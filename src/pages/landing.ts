...
<head>
  ${head(
    isRu ? 'ERA DAL — Мультиагентная научная исследовательская платформа' : 'ERA DAL — Multi-Agent Scientific Research Platform',
    isRu ? 'Надёжные, воспроизводимые результаты через ансамбль LLM, арбитраж и консенсус. Создано для науки, медицины и инженерии.' : 'Reliable, reproducible results through LLM ensemble, arbitration, and consensus. Built for science, medicine, and engineering research.',
    lang
  )}
</head>
<body class="bg-gray-950 text-white">
  ...
  <section class="min-h-screen flex items-center justify-center relative overflow-hidden pt-16">
    ...
    <div class="relative z-10 max-w-6xl mx-auto px-4 text-center">
      <div class="inline-flex items-center space-x-2 bg-blue-500/10 border border-blue-500/30 rounded-full px-4 py-2 mb-8">
        ...
        <span class="text-blue-400 text-sm">${isRu ? 'v1.2.0 — Мультиагентная исследовательская платформа' : 'v1.2.0 — Multi-Agent Scientific Research Platform'}</span>
      </div>

      <h1 class="text-5xl md:text-7xl font-bold mb-6">
        <span class="gradient-text">${isRu ? 'Научные исследования' : 'Scientific Research'}</span><br>
        ${isRu ? 'с мультиагентным ИИ' : 'with Multi-Agent AI'}
      </h1>

      <p class="text-xl text-gray-400 max-w-3xl mx-auto mb-8">
        ${isRu
          ? 'Надёжные, воспроизводимые и верифицированные ответы через ансамбль LLM, арбитраж и самокритику. Создано для науки, медицины и инженерии.'
          : 'Reliable, reproducible, and verified answers through LLM ensemble, arbitration, and self-critique. Built for science, medicine, and engineering research.'}
      </p>
      ...
      <div class="flex flex-col sm:flex-row items-center justify-center gap-4 mb-12">
        <a href="/playground?lang=${lang}" class="bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white px-8 py-4 rounded-xl font-semibold text-lg transition transform hover:scale-105">
          <i class="fas fa-flask mr-2"></i> ${isRu ? 'Попробовать' : 'Try Playground'}
        </a>
        <a href="https://github.com/eukundrotas/ERA-Decision-Arbitration-Layer" class="glass hover:bg-white/10 text-white px-8 py-4 rounded-xl font-semibold text-lg transition">
          <i class="fab fa-github mr-2"></i> ${isRu ? 'GitHub' : 'View on GitHub'}
        </a>
      </div>
  ...
  <section class="py-20">
    <div class="max-w-6xl mx-auto px-4">
      <div class="text-center mb-16">
        ...
        <p class="text-gray-400">${isRu ? 'Инструмент для исследователей, учёных и научных групп' : 'Tools for researchers, scientists, and academic teams'}</p>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="glass rounded-2xl p-6">
          <i class="fas fa-file-alt text-3xl text-purple-500 mb-4"></i>
          <h3 class="text-xl font-semibold mb-2">${isRu ? 'Рецензирование' : 'Peer Review'}</h3>
          <p class="text-gray-400">${isRu ? 'Автоматизированная оценка научных работ с проверкой методологии и выявлением противоречий.' : 'Automated evaluation of research papers with methodology validation and contradiction detection.'}</p>
        </div>
        <div class="glass rounded-2xl p-6">
          <i class="fas fa-dna text-3xl text-green-500 mb-4"></i>
          <h3 class="text-xl font-semibold mb-2">${isRu ? 'Биомедицинские исследования' : 'Biomedical Research'}</h3>
          <p class="text-gray-400">${isRu ? 'Анализ геномных данных, клинических исследований и генерация гипотез с количественной оценкой надёжности.' : 'Genomic data analysis, clinical study interpretation, and hypothesis generation with quantified reliability.'}</p>
        </div>
        <div class="glass rounded-2xl p-6">
          <i class="fas fa-book-open text-3xl text-yellow-500 mb-4"></i>
          <h3 class="text-xl font-semibold mb-2">${isRu ? 'Обзор литературы' : 'Literature Review'}</h3>
          <p class="text-gray-400">${isRu ? 'Систематический синтез публикаций с обнаружением консенсуса и противоречий между источниками.' : 'Systematic synthesis of publications with consensus discovery and contradiction detection across sources.'}</p>
        </div>
        <div class="glass rounded-2xl p-6">
          <i class="fas fa-vials text-3xl text-cyan-500 mb-4"></i>
          <h3 class="text-xl font-semibold mb-2">${isRu ? 'Воспроизводимость' : 'Experiment Reproducibility'}</h3>
          <p class="text-gray-400">${isRu ? 'Верификация воспроизводимости результатов через независимый ансамбль агентов-рассуждателей.' : 'Verify reproducibility of findings through an ensemble of independent reasoning agents.'}</p>
        </div>
      </div>
    </div>
  </section>
    ...
    <div class="max-w-6xl mx-auto px-4">
      <div class="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
        ...
        <div>
          ...
          <div class="text-gray-400">${isRu ? 'Науч. базы данных' : 'Research Databases'}</div>
        </div>
        <div>
  ...
  <section class="py-20">
    <div class="max-w-4xl mx-auto px-4 text-center">
      <h2 class="text-3xl md:text-4xl font-bold mb-6">${isRu ? 'Готовы ускорить ваши исследования?' : 'Ready to Accelerate Your Research?'}</h2>
      <p class="text-gray-400 mb-8 text-lg">${t.ctaSubtitle}</p>
      <div class="flex flex-col sm:flex-row items-center justify-center gap-4">
        <a href="/playground?lang=${lang}" class="bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white px-8 py-4 rounded-xl font-semibold text-lg transition transform hover:scale-105">
          <i class="fas fa-flask mr-2"></i> ${isRu ? 'Начать исследование' : 'Start Research'}
        </a>
        <a href="/docs?lang=${lang}" class="glass hover:bg-white/10 text-white px-8 py-4 rounded-xl font-semibold text-lg transition">
          <i class="fas fa-book mr-2"></i> ${isRu ? 'Документация' : 'Documentation'}
        </a>
      </div>
    </div>
  </section>
